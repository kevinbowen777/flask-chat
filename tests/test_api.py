...
class APITestCase(unittest.TestCase):
    ...
    def test_posts(self):
        ...
        response = self.client.post(
            "/api/v1/posts/", headers=self.get_api_headers("john@example.com", "cat"), data=json.dumps({"body": ""})
        )
        ...
        response = self.client.get(
            "/api/v1/users/{}/posts/".format(u.id), headers=self.get_api_headers("john@example.com", "cat")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        ...
        response = self.client.get(
            "/api/v1/users/{}/timeline/".format(u.id), headers=self.get_api_headers("john@example.com", "cat")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertIsNotNone(json_response.get("posts"))
        ...
        response = self.client.put(
            url, headers=self.get_api_headers("john@example.com", "cat"), data=json.dumps({"body": "updated body"})
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual("http://localhost" + json_response["url"], url)
    ...
    def test_users(self):
        ...
        # get users
        response = self.client.get(
            "/api/v1/users/{}".format(u1.id), headers=self.get_api_headers("susan@example.com", "dog")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
        self.assertEqual(json_response["username"], "john")
        response = self.client.get(
            "/api/v1/users/{}".format(u2.id), headers=self.get_api_headers("susan@example.com", "dog")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
    ...
    def test_comments(self):
        ...
        response = self.client.get(
            "/api/v1/posts/{}/comments/".format(post.id), headers=self.get_api_headers("susan@example.com", "dog")
        )
        self.assertEqual(response.status_code, 200)
        json_response = json.loads(response.get_data(as_text=True))
